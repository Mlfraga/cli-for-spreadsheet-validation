...
export const createTxtFile = async (i: number, anvisaData: AnvisaDataResponse, excelData: ExcelData, erroredRows: Array<{row: number,  item: string}>) => {
    ...
    const result = anvisaData.apresentacoes.map(apresentacao => {
      let flag = 0;
      if(apresentacao.componente === excelData.presentation.componente && excelData.presentation.modelo === excelData.presentation.modelo){
        flag += 1;  
      }
      return flag;
    })
